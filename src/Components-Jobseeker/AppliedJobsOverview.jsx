...
export const AppliedJobsOverview = () => {
  ...
  const { id } = useParams();
  const { appliedJobs, setJobs, setAppliedJobs, Alluser, setAlluser } = useJobs();
  ...
  const getFilteredSteps = () => {
    ...
    const initialSteps = statusFlow.slice(0, 3);    
    return [
      ...initialSteps,
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (isRejected) {
        ...
        const currentIdx = statusFlow.findIndex(s => s.label === liveStatus);
        setActiveStep(currentIdx);
      }
    }, 500);
    return () => clearTimeout(timer);
  }, [liveStatus, isRejected, displaySteps.length]);
      ...
      <div className='appliedjobsO-job-card' style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
        <div>
          ...
          <div style={{ marginTop: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span className={`applied-application-status status-${job.status.type}`}>
              {job.status.text}
            </span>
          </div>
      ...
      <div className='AppliedJobs-overview-main'>
        ...
        <div className="status-container">
              ...
              {displaySteps.map((step, index) => {
                ...
                return (
                  <Step key={index} completed={index < activeStep}>
                    <StepLabel
                      optional={<Typography variant="caption" sx={{ color: isStepRejected ? '#d32f2f' : 'inherit' }}>{step.sub}</Typography>}
                      sx={{
                        '& .MuiStepLabel-label': {
                          fontWeight: index === activeStep ? 700 : 400,
                          color: isStepRejected ? '#d32f2f' : (index <= activeStep ? '#1976d2' : 'inherit')
                        },
                        '& .MuiStepIcon-root.Mui-active': {
                          color: isStepRejected ? '#d32f2f' : '#1976d2'
                        }
                      }}
                    >
                      {step.label}
          ...
          <div style={{ marginTop: '40px' }}>
            {isRejected ? (
              <button onClick={() => removeRejectedJob(job.id)} className="btn-remove-red">
                Remove from Applied
              </button>
            ) : (
              <button onClick={() => withdrawApplication(job.id)} className="btn-withdraw-blue">
